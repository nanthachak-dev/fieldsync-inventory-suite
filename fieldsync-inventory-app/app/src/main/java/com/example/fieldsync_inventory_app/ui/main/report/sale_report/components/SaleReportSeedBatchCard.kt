package com.example.fieldsync_inventory_app.ui.main.report.sale_report.components
...
@Composable
fun SaleReportSeedBatchCard(
    item: SaleReportSeedBatch, // To be changed to SaleReportSeedBatch
    odd: Boolean = false,
        ...
        Row(
            ...
            Column {
                ...
                Text(
                    ...
                    text = "Germinated: ${formatter.format(item.germinated)} Kg | Sold: ${formatter.format(item.germinatedSale)} LAK",
                    fontSize = 12.sp,
                    color = Color.Black,
                ...
                Text(
                    // Generation | Grading | Germination
                    text = "Ungerminated: ${formatter.format(item.ungerminated)} Kg | Sold: ${formatter.format(item.ungerminatedSale)} LAK",
                    fontSize = 12.sp,
                    color = Color.Black,
                    style = TextStyle(lineHeight = 14.sp) // Adjust this value as needed
                )
                // Forth line of details
                Text(
                    // Generation | Grading | Germination
                    text = "Total: ${formatter.format(item.totalGermination)} Kg | Total Sold: ${formatter.format(item.totalGerminationSale)} LAK",
                    fontSize = 12.sp,
                    color = Color.Black,
                    style = TextStyle(lineHeight = 14.sp) // Adjust this value as needed
                )
...
@Composable
private fun PreviewSaleReportSeedBatchCard() {
    RCRCSeedManagerTheme {
        SaleReportSeedBatchCard(
            item = SaleReportSeedBatch(
                varietyId = 1,
                varietyName = "RX-78-2",
                year = 2025,
                seasonId = 1,
                ...
                germinated = 3300.0,
                ungerminated = 2300.0,
                germinatedSale = 20000000.0,
                ungerminatedSale = 12000000.0,
                transactionDate = 1693596800000
            ),
            odd = true,
            onClick = {}
        )
