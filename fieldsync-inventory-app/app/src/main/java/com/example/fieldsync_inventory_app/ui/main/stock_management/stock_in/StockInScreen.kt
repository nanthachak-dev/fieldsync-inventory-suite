...
import com.example.fieldsync_inventory_app.ui.common.components.SeedBatchStockInCard
...
fun StockInScreen(
    ...
    val uiState by viewModel.stockInUiState.collectAsState()
    val resourceUiState by viewModel.resourceUiState.collectAsState()

    val isFirstLaunch by viewModel.isFirstLaunch.collectAsState()

    // Data
    val seedBatches = viewModel.seedBatchDataList

    var showDeleteDialog by remember { mutableStateOf(false) }
    var showEditDialog by remember { mutableStateOf(false) }
    var showClearScreenDialog by remember { mutableStateOf(false) }
    var selectedSeedBatch by remember { mutableStateOf<SeedBatchStockInData?>(null) }
    ...
    LaunchedEffect(resourceUiState.isSuccess) {
        if (resourceUiState.isSuccess) {
            Toast.makeText(context, "Stock in saved successfully!",
                Toast.LENGTH_SHORT).show()
            navController?.popBackStack()
            viewModel.clearStockInScreen()
        }
    }
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        ...
                        navController?.navigate(Screen.AddSeedBatchStockIn.route)
                    }
                ) {
                    Text("Edit")
                }
                ...
                TextButton(
                    onClick = {
                        ...
                        pendingTaskSelection?.let {
                            val selectedTask = StockInTask.fromDisplayName(it)
    ...
    Scaffold(
        topBar = {
            SubTopBar("Stock In", navController)
        },
            ...
            FloatingActionButton(
                onClick = {
                    ...
                    if (uiState.selectedTask == null) {
                        Toast.makeText(context, "Select task first!",
                            Toast.LENGTH_SHORT).show()
                    } else {
                        navController?.navigate(Screen.AddSeedBatchStockIn.route)
                    }
        ...
        LazyColumn(
            ...
            item {
                CompoBoxCustom(
                    uiState.taskList.map { it.displayName },
                    uiState.selectedTask?.displayName,
                    -1,
                    "Select Task",
                    modifier = modifier,
                    onItemSelected = { selectedTaskString ->
                        // If next selectedTask is different from the current selectedTask
                        // and seed batch list is not empty then show dialog to clear screen,
                        if (seedBatches.isNotEmpty() && selectedTaskString != uiState.selectedTask?.displayName) {
                            ...
                            val selectedTask = StockInTask.fromDisplayName(selectedTaskString)
                            viewModel.onTaskChange(selectedTask)
                        }
                    }
                )
            ...
            items(seedBatches, itemContent = {
                SeedBatchStockInCard(
                    ...
                    isPurchase = uiState.selectedTask == StockInTask.PURCHASE
                )
            ...
            if (!seedBatches.isEmpty()) {
                ...
                if (uiState.selectedTask == StockInTask.PURCHASE) {
                    item {
                        TextInputWithSuffix(
                            "Total Price",
                            "LAK",
                            uiState.totalPriceDisplay,
                            onValueChange = {}, // No action needed for readonly
                            modifier = modifier,
                            readOnly = true
                        )
                    }
                ...
                item {
                    CompoBoxCustom(
                        uiState.supplierList,
                        uiState.selectedSupplier,
                        -1,
                        "Supplier:",
                        modifier = modifier,
                        onItemSelected = viewModel::onSupplierChange // Delegate event
                    )
                }
            }
...
// -- Preview --
val previewViewModel = PreviewStockInViewModel()

@Preview(showBackground = true)
@Composable
fun PreviewStockInScreen() {
    RCRCSeedManagerTheme {
        StockInScreen(previewViewModel)
    }
}