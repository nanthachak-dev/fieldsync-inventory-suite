...
fun CustomerFormScreen(
    viewModel: ICustomerViewModel = hiltViewModel<CustomerViewModel>(),
    navController: NavController? = null
) {
    val resourceUiState by viewModel.resourceUiState.collectAsState()
    val uiState by viewModel.customerFormUiState.collectAsState()
    ...
    Scaffold(
        topBar = {
            SubTopBar(
                title = if (uiState.isEditing) "Update Customer [Id: #${viewModel.selectedCustomer?.id}]" else "Add New Customer",
        ...
        LazyColumn(
            ...
            item {
                val customerTypeNames = uiState.customerTypes.map { it.name }
                val selectedIndex = uiState.customerTypes.indexOf(uiState.selectedCustomerType)
                
                CompoBoxCustom(
                    list = customerTypeNames,
                    selectedItem = uiState.selectedCustomerType?.name,
                    startIndex = selectedIndex,
                    label = "Customer Type",
                    onItemSelected = { name ->
                        val selectedType = uiState.customerTypes.find { it.name == name }
                        selectedType?.let { viewModel.onCustomerTypeChange(it) }
                    },
            ...
            item {
                Button(
                    ...
                    Text(
                        text = if (uiState.isEditing) "Update Customer" else "Add New Customer",
                        color = Color.White,
                        fontSize = 18.sp,
...
// -- Preview --
private val previewViewModel = PreviewCustomerFormViewModel()

@androidx.compose.ui.tooling.preview.Preview(showBackground = true)
@Composable
private fun PreviewCustomerFormScreen() {
    CustomerFormScreen(previewViewModel)
}