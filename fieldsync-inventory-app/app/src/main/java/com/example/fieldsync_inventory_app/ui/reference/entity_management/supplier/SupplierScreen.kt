...
import com.example.fieldsync_inventory_app.ui.reference.entity_management.supplier.components.SupplierCard
...
fun SupplierScreen(
    ...
    val supplierUiState by viewModel.supplierUiState.collectAsState()

    val isFirstLaunch by viewModel.isFirstLaunch.collectAsState()
    val suppliers = supplierUiState.suppliers
    ...
    Scaffold(
        topBar = {
            SubTopBar("Supplier", navController)
        },
        containerColor = Color(0xFF3D3C31),
        floatingActionButton = {
            FloatingActionButton(
                onClick = {
                    viewModel.onAddNewSupplierClick()
                    navController?.navigate(Screen.SupplierForm.route)
                },
            ...
            LazyColumn(
                ...
                items(filteredList, itemContent = {
                    SupplierCard(
                        it,
                        onCardClicked = {
                            viewModel.onEditSupplierItemClick(it)
                            navController?.navigate(Screen.SupplierForm.route)
                        },
                        onDeleteClicked = {
                            selectedSupplier = it
    ...
    if (showDeleteDialog) {
        AlertDialog(
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        selectedSupplier?.let { viewModel.onDeleteSupplierItemClick(it) }
...
// -- Preview --
private val previewViewModel = PreviewSupplierViewModel()

@androidx.compose.ui.tooling.preview.Preview(showBackground = true)
@Composable
private fun PreviewSupplierScreen() {
    SupplierScreen(previewViewModel)
}